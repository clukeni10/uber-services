...
import {
  LuFileText, LuCalendar, LuCreditCard, LuDownload, LuUser,
} from "react-icons/lu";
...
export default function WorkerInvoices() {
  ...
  return (
    <Box display="flex" minH="100vh" bg="gray.50">
      ...
      <Box w="100%" flex="1" ml={{ base: "0", md: sidebarW }} transition="margin 0.25s ease">
        {/* Hero */}
        ...
        <Box maxW="1100px" mx="auto" px="6" py="8">
          {loading ? (
            ...
            <Center h="300px">
              <VStack gap="3">
                <Flex w="64px" h="64px" borderRadius="full" bg={`${blue}15`} color={blue}
                  alignItems="center" justifyContent="center">
                  <LuFileText size={28} />
                </Flex>
                <Text color="gray.400" fontWeight="medium">Ainda não tens faturas</Text>
                <Text color="gray.300" fontSize="sm">As faturas aparecem após os clientes pagarem os serviços</Text>
              </VStack>
            </Center>
          ) : (
            <VStack gap="4" align="stretch">
              {invoices.map((invoice) => {
                ...
                return (
                  <Box key={invoice.id} bg={white} borderRadius="2xl" border="1px solid"
                    ...
                    <HStack justify="space-between" align="flex-start" flexWrap="wrap" gap="3">
                      <HStack gap="4" align="flex-start" flex="1">
                        <Flex w="48px" h="48px" borderRadius="xl" flexShrink={0}
                          bg={`${blue}15`} color={blue} alignItems="center" justifyContent="center">
                          <LuFileText size={20} />
                        </Flex>
                        <VStack align="flex-start" gap="1.5" flex="1" minW="0">
                          <HStack gap="2" flexWrap="wrap">
                            <Text fontWeight="bold" fontSize="sm" color="gray.800">{invoice.reference}</Text>
                            <Badge bg={palette.bg} color={palette.color} borderRadius="full" px="2" fontSize="10px">
                              {methodLabel[invoice.method] ?? invoice.method}
                            </Badge>
                          </HStack>
                          <Text fontSize="xs" color="gray.500" >{invoice.description}</Text>
                          <HStack gap="4" flexWrap="wrap">
                            <HStack gap="1.5" color="gray.400">
                              <LuUser size={12} />
                              <Text fontSize="xs">{invoice.client?.name ?? "—"}</Text>
                            </HStack>
                            <HStack gap="1.5" color="gray.400">
                              <LuCalendar size={12} />
                              <Text fontSize="xs">
                      ...
                      <VStack align="flex-end" gap="2" flexShrink={0}>
                        <HStack gap="1.5">
                          ...
                          <Text fontSize="lg" fontWeight="extrabold" color="#10B981">
                        ...
                        <Button size="sm" variant="outline" borderRadius="lg"
                          borderColor={blue} color={blue} _hover={{ bg: `${blue}10` }}
