...
import {
   LuWallet, LuStar, LuClock,
  LuCircleCheck, LuTrendingUp, LuBell, LuUser, LuHand
} from "react-icons/lu";
...
export default function WorkerDashboard() {
  ...
  return (
    <Box display="flex" minH="100vh" bg="gray.50">
      ...
      <Box flex="1" ml={{ base: "0", md: sidebarW }} transition="margin 0.25s ease" overflow="auto">
        <Box maxW="1100px" mx="auto" px={{ base: 4, md: 8 }} py="8">

          {/* Top bar */}
          <HStack justify="space-between" mb="8">
            <VStack align="flex-start" gap="0">
              <Text fontSize="sm" color="gray.400">Bem-vindo de volta <LuHand color={highlights} size="20"/> </Text>
            ...
            <Flex
              ...
              shadow="sm" cursor="pointer" _hover={{ bg: "gray.50" }}
            >
          ...
          <Grid templateColumns={{ base: "repeat(2,1fr)", md: "repeat(4,1fr)" }} gap="4" mb="8">
            {[
              { label: "Ganhos este mês", value: `${Number(stats?.earnings?.month ?? 0).toFixed(2)} Kz`, icon: LuWallet,     color: blue      },
              { label: "Ganhos totais",   value: `${Number(stats?.earnings?.total ?? 0).toFixed(2)} Kz`, icon: LuTrendingUp, color: "#10B981" },
              { label: "Serviços feitos", value: stats?.services?.completed ?? 0,                         icon: LuCircleCheck,color: "#8B5CF6" },
              { label: "Avaliação média", value: Number(stats?.rating?.avg ?? 0).toFixed(1),              icon: LuStar,       color: "#F59E0B" },
            ].map((s) => {
              const Icon = s.icon;
              return (
          ...
          <Grid templateColumns={{ base: "1fr", lg: "1fr 1.6fr" }} gap="6">
            ...
            <Box bg={white} borderRadius="2xl" border="1px solid" borderColor="gray.100" shadow="sm" overflow="hidden">
              <HStack justify="space-between" px="5" py="4" borderBottom="1px solid" borderColor="gray.100">
                <Text fontWeight="bold" color="gray.800">Estado dos Serviços</Text>
                <Text fontSize="xs" color={blue} fontWeight="semibold" cursor="pointer"
              ...
              <VStack gap="0" divideY="1px" divideColor="gray.100">
                {[
                  { label: "Pendentes",  value: stats?.services?.pending,   color: "#F59E0B" },
                  { label: "Aceites",    value: stats?.services?.accepted,  color: "#3B82F6" },
                  { label: "Em curso",   value: stats?.services?.active,    color: "#8B5CF6" },
                  { label: "Concluídos", value: stats?.services?.completed, color: "#10B981" },
                  { label: "Cancelados", value: stats?.services?.cancelled, color: "#EF4444" },
                ].map((s) => (
                  <HStack key={s.label} px="5" py="3.5" w="full" justify="space-between">
                    <HStack gap="2.5">
                      <Box w="8px" h="8px" borderRadius="full" bg={s.color} flexShrink={0} />
                      <Text fontSize="sm" color="gray.600">{s.label}</Text>
                    </HStack>
                    <Text fontSize="sm" fontWeight="bold" color="gray.800">{s.value ?? 0}</Text>
                  </HStack>
                ))}
              </VStack>
            </Box>
            ...
            <Box bg={white} borderRadius="2xl" border="1px solid" borderColor="gray.100" shadow="sm" overflow="hidden">
              <HStack justify="space-between" px="5" py="4" borderBottom="1px solid" borderColor="gray.100">
                <Text fontWeight="bold" color="gray.800">Atividade Recente</Text>
                <Text fontSize="xs" color={blue} fontWeight="semibold" cursor="pointer"
                  onClick={() => navigate("/worker/services")}>
                  Ver todos
                </Text>
              ...
              {!stats?.recentServices?.length ? (
                <Center h="150px">
                  <Text color="gray.400" fontSize="sm">Sem atividade recente</Text>
                </Center>
              ) : (
                <VStack gap="0" divideY="1px" divideColor="gray.100">
                  {stats.recentServices.map((s: any) => {
                    ...
                    return (
                      <HStack key={s.id} px="5" py="4" w="full" justify="space-between"
                        _hover={{ bg: "gray.50" }} transition="background 0.15s">
                        <HStack gap="3">
                          {s.client_image ? (
                            <Box w="36px" h="36px" borderRadius="full" overflow="hidden" flexShrink={0}>
                              <img src={s.client_image} alt={s.client_name}
                                style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                            </Box>
                          ) : (
                            <Flex w="36px" h="36px" borderRadius="full" bg={`${blue}15`} color={blue}
                          ...
                          <VStack align="flex-start" gap="0">
                            ...
                            <Text fontSize="xs" color="gray.400">{s.description}</Text>
                          </VStack>
                        </HStack>
                        <Badge bg={config?.bg} color={config?.color} borderRadius="full" px="2" fontSize="10px">
                          {config?.label}
                        </Badge>
                      </HStack>
                    );
          ...
          {(stats?.services?.pending ?? 0) > 0 && (
            <Box
              mt="6" bg={blue} borderRadius="2xl" p="6"
              position="relative" overflow="hidden" cursor="pointer"
              onClick={() => navigate("/worker/services")}
              _hover={{ opacity: 0.95 }}
            >
              <Box position="absolute" top="-30px" right="-30px" w="140px" h="140px"
                borderRadius="full" bg="whiteAlpha.100" />
              <HStack justify="space-between">
                <HStack gap="4">
                  <Flex w="48px" h="48px" borderRadius="xl" bg="whiteAlpha.200"
                    alignItems="center" justifyContent="center" color={white}>
                    <LuClock size={22} />
                  </Flex>
                  <VStack align="flex-start" gap="0">
                    <Text color={white} fontWeight="bold" fontSize="md">
                      Tens {stats?.services?.pending} pedido{stats?.services?.pending > 1 ? "s" : ""} pendente{stats?.services?.pending > 1 ? "s" : ""}
                    </Text>
                ...
                <Box
                  bg={white} color={blue} px="4" py="2" borderRadius="xl"
